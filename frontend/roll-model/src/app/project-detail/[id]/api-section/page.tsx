...
export default function ApiSectionPage() {
  ...
  const { apiStatus, endpoint, inputSchema } = projectDetailApi;
  return (
    <>
      <div>프로젝트 상세 (api)</div>
      <ApiDownloadCard></ApiDownloadCard>
      <ApiStatusCard apiStatus={apiStatus}></ApiStatusCard>
      <ApiEndpointCard endpoint={endpoint}></ApiEndpointCard>
      <ApiExamplesCard inputSchema={inputSchema}></ApiExamplesCard>
    </>
  );
}
