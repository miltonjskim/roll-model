...
const InputProjectMetaDataPage = () => {
	...
	const [type, setType] = useState<projectCategory>("REGRESSION");
	const [isPublic, setIsPublic] = useState(true);

	const handleSubmit = async () => {
		try {
			// TODO: 백엔드 서버 배포 시 아래 주석 삭제
		...
		<div>
			...
			<div>
				...
				<div>
					...
					<Select
						value={domain}
						onValueChange={(val) => setDomain(val as projectDomain)}>
						<SelectTrigger>
				...
				<div>
					...
					<Select
						value={type}
						onValueChange={(val) => setType(val as projectCategory)}>
						<SelectTrigger>
							<SelectValue placeholder="선택하세요" />
						</SelectTrigger>
						<SelectContent>
