...
const VersionGraphDagre = ({
  versionHistory,
  selectedVersion,
  onSelectVersion,
  selectedPipeline,
}: VersionGraphDagreProps) => {
  ...
  // Y 위치 맵 생성 함수
  const createYPositionMap = useCallback(
    (timeOrderedVersions: VersionHistory[], spacing: number) => {
  ...
  // 부모별 자식 노드 그룹화 함수
  const createChildrenByParentMap = useCallback(
  ...
  // 노드 X 위치 계산 함수
  const calculateNodeXPosition = useCallback(
    (
      node: Node,
      xPos: number,
      ...
      if (parentId) {
        ...
        if (siblings.length === 1) {
          ...
          if (nodeIndex !== -1) {
            const offset =
              (nodeIndex - (siblings.length - 1) / 2) * NODE_OFFSET;
  ...
  const applyTreeBasedLayout = useCallback(
    (nodes: Node[], edges: Edge[]) => {
      ...
      const majorVersions = [
        ...new Set(nodes.map((node) => Math.floor(parseFloat(node.id)))),
      ].sort();
      ...
      const positionChildren = (parentId: string) => {
        ...
        children.forEach((childNode, index) => {
          ...
          if (
            isExactMajorVersion &&
            childMajorVersion !== parseFloat(parentId)
          ) {
  ...
  useEffect(() => {
    ...
    const yPositionMap = createYPositionMap(
      timeOrderedVersions,
      INITIAL_Y_SPACING
    );
    ...
    // 시간 기반 커스텀 레이아웃 적용
    const { nodes: layoutedNodes, edges: layoutedEdges } = applyTreeBasedLayout(
      graphNodes,
      graphEdges
    );
    ...
    const positionChildren = (parentId: string) => {
      ...
      children.forEach((childNode, index) => {
        ...
        if (isExactMajorVersion && childMajorVersion !== parseFloat(parentId)) {
          ...
          if (children.length === 1) {
            // 자식이 하나인 경우 부모와 같은 X 위치에 배치
            const nodeToUpdate = updatedNodes.find(
              (n) => n.id === childNode.id
            );
            if (nodeToUpdate) {
  ...
  return (
    <div
      className="w-full border border-2 border-gray-200 rounded-lg"
      style={{
    ...
    >
      ...
      >
        ...
        <Panel position="top-left" className="flex flex-col gap-2">
          ...
          <button
            onClick={onLayout}
            className="px-4 py-2 bg-[theme(primary-black)] hover:bg-gray-01 duration-300 ease-in text-white rounded focus:outline-none"
          >
            재정렬
          </button>
          {/* 범례 */}
          <div className="legend flex flex-col gap-2 p-3 bg-white rounded border border-gray-200 shadow-sm">
            <div className="text-sm font-medium mb-1">버전 상태</div>
            <div className="flex items-center">
              <div className="w-4 h-4 rounded-full bg-[theme(color-blue-02)] mr-1"></div>
              <span className="text-xs">선택한 버전</span>
            </div>
            <div className="flex items-center">
              <div className="w-4 h-4 rounded-full bg-[theme(color-green-02)] mr-1"></div>
              <span className="text-xs">최신 버전</span>
            </div>
            <div className="flex items-center">
              <div className="w-4 h-4 rounded-full bg-[theme(color-rose-02)] mr-1"></div>
              <span className="text-xs">나의 버전</span>
            </div>
            <div className="flex items-center">
              <div className="w-4 h-4 rounded-full bg-[theme(color-gray-02)] mr-1"></div>
