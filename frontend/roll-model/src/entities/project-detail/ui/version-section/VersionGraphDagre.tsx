...
import { Pipeline } from '../../model/versionTypes';
...
const VersionNode = ({ data, selected }: any) => {
  const { label, version, isLatest, isPublic, isDeleted, isOwned, onClick } = data;
...
const VersionGraphDagre = ({ pipelines, selectedVersion, onSelectVersion, selectedPipeline }: VersionGraphDagreProps) => {
  ...
  const latestVersion = useMemo(() => {
    ...
    const mostRecentVersion = pipelines.reduce((latest, current) => {
  ...
  // Y 위치 맵 생성 함수
  const createYPositionMap = useCallback((timeOrderedVersions: Pipeline[], spacing: number) => {
  ...
  const applyTreeBasedLayout = useCallback(
    (nodes: Node[], edges: Edge[]) => {
      ...
      // console.log('뭐지', childrenByParent.get('1.0'));
      ...
      const positionChildren = (parentId: string) => {
        ...
        // console.log('실행됨?', parentId, children);

        if (children.length === 0) return;
        // console.log('이건실행됨?');
        // console.log(parentId, '의 자식', children);
      ...
      // 메이저 버전 찾기 (예: 1.0, 2.0, 3.0)
      const majorNodeIds = nodes
        .filter((node) => {
          const nodeVersion = parseFloat(node.id);
          const majorVersion = Math.floor(nodeVersion);
          return nodeVersion === majorVersion; // 1.0, 2.0, 3.0 같은 정확한 메이저 버전인지 확인
        })
        .map((node) => node.id); // 실제 ID 문자열("1.0", "2.0" 등)을 사용

      // 메이저 버전 노드부터 시작하여 자식 노드 위치 계산
      majorNodeIds.forEach((majorNodeId) => {
  ...
  useEffect(() => {
    ...
    pipelines.forEach((pipeline) => {
      const yPosition = yPositionMap.get(pipeline.version) || 0;

      graphNodes.push({
        id: pipeline.version.toString(),
        type: 'versionNode',
        data: {
          label: pipeline.version,
          version: pipeline.version,
          isLatest: pipeline.version === latestVersion,
          isPublic: pipeline.publicYn,
          isDeleted: pipeline.deletedYn,
          isOwned: pipeline.ownerYn,
          onClick: () => onSelectVersion(pipeline.version),
        },
        ...
        selected: pipeline.version === selectedVersion,
      });

      // 부모 버전으로 연결되는 엣지 (자식 → 부모 방향)
      // 자기 자신을 가리키는 엣지는 생성하지 않음
      if (pipeline.version !== pipeline.parent) {
        // console.log('다름 있음');

        // 엣지 생성 시 스타일 명시적 지정
        graphEdges.push({
          id: `e${pipeline.version}-${pipeline.parent.toString()}`,
          source: pipeline.parent.toString(),
          target: pipeline.version.toString(),
  ...
  }, [pipelines, selectedVersion, latestVersion, onSelectVersion, reactFlowInstance, getTimeOrderedVersions, createYPositionMap, applyTreeBasedLayout]);
  ...
  const onLayout = useCallback(() => {
    ...
    updatedNodes.forEach((node) => {
      ...
      // Y 위치 업데이트
      node.position.y = yPositionMap.get(node.id) || 0;
  ...
  }, [nodes, edges, pipelines, setNodes, reactFlowInstance, getTimeOrderedVersions, createYPositionMap, createChildrenByParentMap]);
  return (
    <div
