...
async def get_source_pipeline(db, pipeline_id, project_id, pipeline_service:PipelineService):
    ...
    logger.info(f"source_pipeline_detailsssssssssssssssssssssssssss: {source_pipeline_details}")
...
async def determine_target_project(db, project_id, member_id, source_pipeline, original_project_id, original_project_owner_id):
    """
    타겟 프로젝트를 결정합니다.
    ...
    try:
        ...
        if not user_project_ids:
            ...
            new_project_id = await create_new_project(db, original_project_id, member_id)
            return new_project_id if new_project_id else None
        ...
        ancestor_query = text("""
            WITH RECURSIVE ancestors AS (
                SELECT pipeline_id, parent_pipeline_id, project_id, 0 AS depth
                FROM pipelines
                ...
                AND p.pipeline_id != a.pipeline_id  -- 자기 참조 방지
                AND p.parent_pipeline_id IS NOT NULL  -- NULL 참조 방지
                AND a.depth < 10  -- 깊이 제한
            )
            SELECT pipeline_id
            FROM ancestors
            WHERE depth > 0
            ...
            try:
                if len(all_lineage_ids) == 1:
                    ...
                    related_project_query = text("""
                        ...
                        AND prj.member_id = :member_id
                        GROUP BY p.project_id
                        ORDER BY COUNT(p.pipeline_id) DESC
                        LIMIT 1
                    """)
...
async def create_new_project(db, original_project_id, member_id):
    ...
    try:
        ...
        # 프로젝트 생성 로그
        logger.info(f"새 프로젝트가 생성되었습니다. ID: {new_project_id}, 소유자: {member_id}")
...
async def save_new_pipeline(db, new_pipeline, pipeline_id, target_project_id, member_id,
                           source_pipeline, status, new_version, pipeline_service:PipelineService):
    """
...
async def prepare_response_data(new_pipeline_id, target_project_id, pipeline_id, original_project_id,
