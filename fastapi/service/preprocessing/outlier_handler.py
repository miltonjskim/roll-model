...
class OutlierHandler:
    ...
    def handle_outliers_imputation(self, column, method='MEAN', detection='ZSCORE', threshold=3.0):
        """
        이상치 처리 (대체)
        ...
        if method == 'THRESHOLD':
            ...
            min_threshold = detection_result.get("lower_bound",
                                                 detection_result["mean"] - threshold * detection_result["std"])
            max_threshold = detection_result.get("upper_bound",
                                                 detection_result["mean"] + threshold * detection_result["std"])
    ...
    def handle_outliers_remove(self, column, method='ROW_REMOVE', detection='ZSCORE', threshold=3.0):
        """
        이상치 제거
    ...
    def detect_outliers_iqr(self, column, multiplier=1.5):
        ...
        if column not in self.df.columns:
            return {
                "error": f"컬럼 '{column}'이 데이터에 존재하지 않습니다."
            }
    ...
    def handle_outliers_detection(self, column=None, detection='ZSCORE', threshold=3.0):
        ...
        # 전체 숫자형 컬럼에 대해 탐지
            ...
            for col in numeric_columns:
                try:
                    if detection == 'IQR':
                        detection_result = self.detect_outliers_iqr(col, multiplier=threshold)
                    elif detection == 'ZSCORE':
                        detection_result = self.detect_outliers_zscore(col, threshold=threshold)
                    else:
                        continue
            ...
            result = {
                ...
                "maxThreshold": None,
                "outlierIndices": sorted(list(all_outlier_indices)),  # 중복 제거된 전체 이상치 인덱스
