...
class ColumnTypeInferrer:
    ...
    @execution_time
    def infer_types(
        self, df: pd.DataFrame, exclude_types: List[str] = None
    ) -> List[Dict[str, str]]:
        """
        ...
        if len(series) > self.sample_size:
            ...
            if remaining_size > 0:
                remaining_indices = np.random.choice(
                    range(100, len(series)), size=remaining_size, replace=False
                )
...
def process_columns_with_inference(
    df: pd.DataFrame, exclude_types: List[str] = None, sample_size: int = 1000
) -> tuple:
    """
