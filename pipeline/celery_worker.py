...
@app.task
def train_model_task(data_path: str, model_params: dict, save_path=None, target_column=None):
    # 기본값 설정
    if target_column is None:
        target_column = DEFAULT_TARGET_COLUMN

    print(f"[Task] Start training model with data: {data_path}")
    ...
    try:
        ...
        # 데이터 로딩
        df = pd.read_csv(data_path)  # 추후 MinIO 연동 필요
